...
export const useMembers = () => {
    ...
    const getMembers = async () => {
        try {
            const response = await axios.get(baseUrl + '/getMembers', { withCredentials: true })
    ...
    const changePassword = async (email, newPassword, onSuccess) => {
        try {
            const response = await axios.post(baseUrl + '/changePassword', { email: email, new_password: newPassword }, { withCredentials: true })
    ...
    const removeMember = async (email, onSuccess) => {
        try {
            await axios.post(baseUrl + '/removeMember', { email: email }, { withCredentials: true })
            getMembers()
            onSuccess()
        } catch (err) {
            console.error(err)
    ...
    return { getMembers, filterMembers, filteredMemberList, addMember, changePassword, removeMember }
}