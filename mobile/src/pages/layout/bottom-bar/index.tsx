...
function BottomButton({ icon, num, onPress, pageNum }: BottomButtonProps) {
    return (
        <IconButton
            onPress={() => onPress(num)}
            bg={pageNum === num ? 'primary' : 'transparent'}
            borderRadius="full"
            icon={icon}
        />
    );
...
export default function BottomBar({ pagerRef }: BBProps) {
    const theme = useTheme();
    const iconColor = (num: number) =>
        pageNum === num ? 'secondary' : 'primary';
    const iconSize = 'lg';
    ...
    const handlePress = (toPage: number) => {
        ...
        if (pagerRef?.current) {
            pagerRef.current.setPage(toPage);
        }
    };
